...
const OrderDetails = () => {
  ...
  const generatePDF = () => {
    ...
    doc.rect(startX, borderTop, 170, 200, "F");
    ...
    doc.roundedRect(startX, borderTop, 170, borderHeight, 5, 5);
    doc.setLineWidth(0.2);
    doc.roundedRect(startX + 3, borderTop + 3, 164, borderHeight - 6, 4, 4);
  ...
  if (error) {
    return (
      <div style={{ textAlign: "center", fontSize: "20px", color: "#c0392b", marginTop: "100px" }}>
        {error}
      </div>
    );
  }
      ...
      >
        ...
        <ul style={{ listStyle: "none", paddingLeft: 0 }}>
          {order.items.map((item, index) => (
            <li
              key={item.foodId || `${item.foodId}-${index}`}
              style={{
            ...
            >
              <p>
                <strong>Food :</strong> {item.foodId}
              </p>
              <p>
